...
export default function Dashboard() {
  ...
  useEffect(() => {
    api.get('/services').then((res) => setCounts((c) => ({ ...c, services: res.data.data.length }))).catch(() => {});
    api.get('/invoices').then((res) => {
      const invoices = res.data.data ?? [];
      const unpaid = invoices.filter((i: Invoice) => i.status === 'unpaid' || i.status === 'overdue');
      setUnpaidInvoices(unpaid);
      setCounts((c) => ({ ...c, unpaidInvoices: unpaid.length }));
    }).catch(() => {});
    api.get('/public/bank').then((r) => setBank(r.data.data ?? {})).catch(() => {});
  }, []);
      ...
      {hasBank && (
        <div className="rounded-2xl border-2 border-emerald-200 bg-white p-6 shadow-sm">
          ...
          <div className="space-y-3">
            {bank.bank_name && (
              <Row label="Banka" value={bank.bank_name} onCopy={copy} copied={copied === 'bank'} copyKey="bank" />
            )}
            {bank.bank_account_holder && (
              <Row label="Hesap Sahibi" value={bank.bank_account_holder} onCopy={copy} copied={copied === 'holder'} copyKey="holder" />
            )}
            {bank.bank_iban && (
              <Row label="IBAN" value={bank.bank_iban} onCopy={copy} copied={copied === 'iban'} copyKey="iban" mono />
            )}
            {bank.bank_branch && (
              <Row label="Şube" value={bank.bank_branch} onCopy={copy} copied={copied === 'branch'} copyKey="branch" />
            )}
          </div>
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-amber-800">
                  Havale/EFT açıklamasına mutlaka <b>fatura/sipariş numaranızı</b> yazın.
                  Aksi takdirde ödemeniz hesabınızla eşleştirilemez ve hizmetiniz aktive edilemez.
                </p>
                {unpaidInvoices.length > 0 && (
                  <p className="text-xs text-amber-700 mt-2">
                    Bekleyen fatura no(ları):{' '}
                    {unpaidInvoices.map((i) => (
                      <code key={i.id} className="inline-block mx-1 px-2 py-0.5 bg-white rounded font-bold">
                        {i.invoiceNum}
                      </code>
                    ))}
                  </p>
...
function Row({
  label, value, onCopy, copied, copyKey, mono,
}: {
  label: string;
  value: string;
  ...
  return (
    <div className="flex items-center justify-between border-b border-slate-100 pb-2 last:border-0 last:pb-0">
      ...
      <div className="flex-1 text-right">
        <span className={mono ? 'font-mono font-semibold text-slate-800 tracking-wide' : 'font-medium text-slate-800'}>
          {value}
        </span>
      </div>
      <button
